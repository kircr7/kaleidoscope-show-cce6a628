...
const largeItems: LargeItem[] = [
  ...
  { image: outputPoster, label: "Постеры" },
  { image: outputPlan, label: "Планы" },
  { image: outputSpec, label: "Спецификации" },
  { image: outputSection, label: "Разрезы" },
];
...
const IconCarousel = ({ side }: Props) => {
  ...
  if (isSmall) {
    ...
    return (
      <div className="flex items-center gap-3 md:gap-4 animate-carousel-to-right">
        {track.map((item, i) => (
          <div key={i} className="flex-shrink-0">
            <div className="w-20 h-28 md:w-24 md:h-34 rounded-lg bg-black border border-[hsl(0_0%_30%)]/50 p-1.5 flex flex-col gap-1.5 transition-transform duration-200 hover:scale-110 active:scale-95 shadow-lg shadow-black/50">
  ...
  return (
    <div className="flex items-center gap-3 md:gap-4 animate-carousel-from-center">
      {track.map((item, i) => (
        <div key={i} className="flex-shrink-0">
          <div className="w-36 h-32 md:w-44 md:h-40 bg-[hsl(0_0%_4%)] border border-[hsl(0_0%_15%)]/30 p-1.5 shadow-xl shadow-black/50 transition-transform duration-200 hover:scale-105 active:scale-95 flex flex-col">
